...
import { TaskManager, type FileTask } from './utils/TaskManager'
import type { AbbrLinkSettings } from './utils/types'
import {
  generateRandomHash,
  generateUniqueHash,
  getExistingAbbrlink
...
export default class AbbrLinkPlugin extends Plugin {
  ...
  private async resolveConflicts(tasks: FileTask[]): Promise<void> {
    const usedHashes = new Set(tasks.map((task) => task.hash).filter(Boolean))
